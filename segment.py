...
def segment(model, recipe, lnadir, batches, bidx):
    ...
    #pp_exe = 'phone_probs'
    #pp_cmd = "%s -b %s -c %s -r %s -o %s -C %s --eval-ming=0.15 -B %i -I %i" %\
    #         (pp_exe, model, "%s.cfg" % model, recipe, lnadir, "%s.gcl" % model, batches, bidx)
    #p = subprocess.Popen(pp_cmd, shell=True)
    #p.wait()

    segment_exe = '/home/mavarjok/wdecoder/segment'
    model_ph = "%s.ph" % model
    model_dur = "%s.dur" % model
    batch_options = "-B %i -I %i" % (batches, bidx)
    seg_cmd = "%s -t -l -s -d %s %s %s %s" % (segment_exe, model_dur, batch_options, model_ph, recipe)
...
def main(argv=None):
    ...
    usage = "usage: %prog [options] MODEL RECIPE BATCHES"
    ...
    for i in range(batches):
        bidx = i+1
        t = threading.Thread(target=segment, args=[model, recipe, lnadir, batches, bidx])
        t.start()
        #threads.append(t)
        t.join()
