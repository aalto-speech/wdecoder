...
{
    conf::Config config;
    config("usage: tdecode [OPTION...] PH LEXICON LM CFGFILE GRAPH LNALIST RESLIST\n")
      ('h', "help", "", "", "display help")
    ...
    try {
        ...
        double total_lp = 0.0;
        int file_count = 0;
        while (getline(lnalistf, lnaline)) {
            getline(reslistf, resline);
            if (!lnaline.length()) continue;
            if (!resline.length()) break;
            cerr << endl << "recognizing: " << lnaline << endl;
            cerr << endl << "tracked result: " << resline << endl;
            int curr_frames;
            double curr_time;
