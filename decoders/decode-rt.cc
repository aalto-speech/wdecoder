...
void
read_config(NgramDecoderRT &d, string cfgfname)
{
...
void join(vector<string> &lnafnames,
          vector<NgramRecognitionRT*> &recognitions,
...
void
recognize_lnas(NgramDecoderRT &d,
               conf::Config &config,
               string lnalistfname,
               ostream &resultf,
               ostream &logf)
{
    ...
    while (getline(lnalistf, lnafname)) {
        ...
        if ((int)recognitions.size() < num_threads) {
            ...
            recognitions.push_back(new NgramRecognitionRT(d));
            results.push_back(new RecognitionResult());
            thread *thr = new thread(&NgramRecognitionRT::recognize_lna_file,
                                     recognitions.back(),
                                     lnafname,
...
{
    conf::Config config;
    config("usage: decode-rt [OPTION...] PH LEXICON LM CFGFILE GRAPH LNALIST\n")
    ('h', "help", "", "", "display help")
