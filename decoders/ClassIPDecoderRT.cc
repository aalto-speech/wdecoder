...
#include "ClassIPDecoderRT.hh"

using namespace std;


ClassIPDecoderRT::ClassIPDecoderRT()
{
    m_iw = 0.0;
    m_class_iw = 0.0;
    m_word_iw = 0.0;
...
void
ClassIPDecoderRT::read_lm(string lmfname)
{
...
void
ClassIPDecoderRT::set_text_unit_id_ngram_symbol_mapping()
{
...
void
ClassIPDecoderRT::read_class_lm(
        string ngramfname,
        string classmfname)
{
...
int
ClassIPDecoderRT::read_class_memberships(
        string fname,
...
ClassIPRecognitionRT::ClassIPRecognitionRT(ClassIPDecoderRT &decoder)
    : Recognition::Recognition(decoder)
{
    cid = static_cast<ClassIPDecoderRT*>(d);
...
void
ClassIPRecognitionRT::reset_frame_variables()
{
    m_token_count = 0;
...
void
ClassIPRecognitionRT::propagate_tokens()
{
    int node_count = 0;
...
void
ClassIPRecognitionRT::prune_tokens(bool collect_active_histories)
{
    vector<ClassIPTokenRT> pruned_tokens;
    ...
    for (unsigned int i=0; i<m_raw_tokens.size(); i++) {
        ClassIPTokenRT &tok = m_raw_tokens[i];
    ...
    for (auto tit = pruned_tokens.begin(); tit != pruned_tokens.end(); tit++) {
        map<int, ClassIPTokenRT> &node_tokens = m_recombined_tokens[tit->node_idx];
...
void
ClassIPRecognitionRT::move_token_to_node(ClassIPTokenRT token,
                            int node_idx,
...
bool
ClassIPRecognitionRT::update_lm_prob(ClassIPTokenRT &token, int word_id)
{
...
double
ClassIPRecognitionRT::class_lm_score(ClassIPTokenRT &token, int word_id)
{
...
void
ClassIPRecognitionRT::get_tokens(vector<Token*> &tokens)
{
    tokens.clear();
    for (auto nit = m_active_nodes.begin(); nit != m_active_nodes.end(); ++nit) {
        map<int, ClassIPTokenRT> &node_tokens = m_recombined_tokens[*nit];
...
void
ClassIPRecognitionRT::add_sentence_ends(vector<Token*> &tokens)
{
    for (auto tit = tokens.begin(); tit != tokens.end(); ++tit) {
        ClassIPTokenRT *token = static_cast<ClassIPTokenRT*>(*tit);
...
string
ClassIPRecognitionRT::get_best_word_history()
{
    vector<Token*> tokens;
    get_tokens(tokens);
...
string
ClassIPRecognitionRT::get_word_history(WordHistory *history)
{
    string result;
