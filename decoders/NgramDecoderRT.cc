...
#include "NgramDecoderRT.hh"

using namespace std;


NgramDecoderRT::NgramDecoderRT()
{
...
void
NgramDecoderRT::read_lm(string lmfname)
{
...
void
NgramDecoderRT::set_text_unit_id_ngram_symbol_mapping()
{
      ...
      m_ngram_state_sentence_begin(decoder.m_ngram_state_sentence_begin)
{
    ...
    tok.preceeding_word_id = m_sentence_begin_symbol_idx;
    ...
    m_recombined_tokens[d->m_decode_start_node][make_pair(tok.preceeding_word_id, tok.last_word_id)] = tok;
    m_total_token_count = 0;
}


void
NgramRecognitionRT::reset_frame_variables()
{
    m_token_count = 0;
...
void
NgramRecognitionRT::propagate_tokens()
{
    int node_count = 0;
...
void
NgramRecognitionRT::prune_tokens(bool collect_active_histories)
{
    vector<NgramTokenRT> pruned_tokens;
    ...
    for (unsigned int i=0; i<m_raw_tokens.size(); i++) {
        NgramTokenRT &tok = m_raw_tokens[i];
    ...
    for (auto tit = pruned_tokens.begin(); tit != pruned_tokens.end(); tit++) {
        map<pair<int,int>, NgramTokenRT> &node_tokens = m_recombined_tokens[tit->node_idx];
        auto bntit = node_tokens.find(make_pair(tit->preceeding_word_id, tit->last_word_id));
        if (bntit != node_tokens.end()) {
            ...
            node_tokens[make_pair(tit->preceeding_word_id, tit->last_word_id)] = *tit;
...
void
NgramRecognitionRT::move_token_to_node(
    NgramTokenRT token,
    int node_idx,
    ...
    bool update_lookahead)
{
    ...
    if (node.word_id != -1) {
        ...
        token.preceeding_word_id = token.last_word_id;
...
void
NgramRecognitionRT::get_tokens(vector<Token*> &tokens)
{
    tokens.clear();
    for (auto nit = m_active_nodes.begin(); nit != m_active_nodes.end(); ++nit) {
        map<pair<int,int>, NgramTokenRT> & node_tokens = m_recombined_tokens[*nit];
...
void
NgramRecognitionRT::add_sentence_ends(vector<Token*> &tokens)
{
    for (auto tit = tokens.begin(); tit != tokens.end(); ++tit) {
        NgramTokenRT *token = static_cast<NgramTokenRT*>(*tit);
...
string
NgramRecognitionRT::get_best_word_history()
{
    vector<Token*> tokens;
    get_tokens(tokens);
...
string
NgramRecognitionRT::get_word_history(WordHistory *history)
{
    string result;
