...
void
read_config(ClassIPDecoderRT &d, string cfgfname)
{
...
void
print_config(ClassIPDecoderRT &d,
             conf::Config &config,
             ostream &outf)
{
...
join(vector<string> &lnafnames,
     vector<ClassIPRecognitionRT*> &recognitions,
...
void
recognize_lnas(ClassIPDecoderRT &d,
               conf::Config &config,
               string lnalistfname,
               ostream &resultf,
               ostream &logf)
{
    ...
    while (getline(lnalistf, lnafname)) {
        ...
        if ((int)recognitions.size() < num_threads) {
            ...
            recognitions.push_back(new ClassIPRecognitionRT(d));
            results.push_back(new RecognitionResult());
            thread *thr = new thread(&ClassIPRecognitionRT::recognize_lna_file,
                                     recognitions.back(),
                                     lnafname,
