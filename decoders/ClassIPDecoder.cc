...
void
ClassIPDecoder::read_phone_model(string phnfname)
{
...
void
ClassIPDecoder::read_duration_model(string durfname)
{
...
void
ClassIPDecoder::read_noway_lexicon(string lexfname)
{
...
void
ClassIPDecoder::read_dgraph(string fname)
{
...
void
ClassIPDecoder::set_hmm_transition_probs()
{
    for (unsigned int i=0; i<m_nodes.size(); i++) {
...
bool descending_node_sort_3(const pair<int, float> &i, const pair<int, float> &j)
{
...
ClassIPDecoder::active_nodes_sorted_by_best_lp(vector<int> &nodes)
{
    ...
    sort(sorted_nodes.begin(), sorted_nodes.end(), descending_node_sort_3);
...
ClassIPDecoder::update_lm_prob(Token &token, int word_id)
{
    ...
    token.lm_node = m_lm.score(token.lm_node, m_subword_id_to_ngram_symbol[word_id], word_lm_prob);
...
void
ClassIPDecoder::print_dot_digraph(vector<Node> &nodes,
                           ostream &fstr)
{
