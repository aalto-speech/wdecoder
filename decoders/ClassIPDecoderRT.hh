...
class ClassIPDecoderRT : public Decoder {
public:
    ClassIPDecoderRT();
    ~ClassIPDecoderRT();
...
class ClassIPRecognitionRT : public Recognition {
public:
    class ClassIPTokenRT : public Token {
    public:
        int lm_node;
        int class_lm_node;
        ClassIPTokenRT():
    ...
    ClassIPRecognitionRT(ClassIPDecoderRT &decoder);
    ...
    void move_token_to_node(ClassIPTokenRT token,
                            int node_idx,
                            float transition_score,
                            bool update_lookahead);
    bool update_lm_prob(ClassIPTokenRT &token, int node_idx);
    double class_lm_score(ClassIPTokenRT &token, int word_id);
    ...
    std::vector<ClassIPTokenRT> m_raw_tokens;
    std::vector<std::map<int, ClassIPTokenRT> > m_recombined_tokens;
    ClassIPDecoderRT *cid;
};
