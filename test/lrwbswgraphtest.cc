...
string lrwb_lexname = string("data/lex");
...
namespace LRWBGraphTestUtils {
    void read_fixtures(LRWBSubwordGraph &swg) {
        swg.read_phone_model(lrwb_amname + ".ph");
        swg.read_lexicon(lrwb_amname,
...
BOOST_AUTO_TEST_CASE(LRWBSubwordGraphTest1)
{
    ...
    lrwb_lexname = "data/LRWB_1.lex";
    LRWBGraphTestUtils::read_fixtures(swg);
