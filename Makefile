cxxflags = -O3 -DNDEBUG -std=gnu++0x -Wall -Wno-write-strings -Wno-unused-function -Lc:\lib64
...
progs_srcs = $(progs:=.cc)
progs_objs = $(progs:=.o)
srcs = conf.cc io.cc Ngram.cc Hmm.cc NowayHmmReader.cc DecoderGraph.cc ConstrainedSWGraph.cc SubwordGraph.cc WordGraph.cc LWBSubwordGraph.cc RWBSubwordGraph.cc LnaReaderCircular.cc Decoder.cc Segmenter.cc Lookahead.cc
objs = $(srcs:.cc=.o)
...
$(progs): %: %.o $(objs)
	$(CXX) $(cxxflags) $< -o $@ $(objs) -lzlib1 -pthread

%: %.o $(objs)
	$(CXX) $(cxxflags) $< -o $@ $(objs) -lzlib1

$(test_progs): %: %.o $(objs) $(test_objs)
	$(CXX) $(cxxflags) $< -o $@ $(objs) $(test_objs) -lcppunit -lzlib1
